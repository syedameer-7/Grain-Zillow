...
export default function EmployeeManagement() {
  ...
  const handleAllEmployeesChange = (e) => {
...
Priority: ${priority}
${allEmployees ? "To all employees" : `To: ${selectedEmployees.join(", ")}`}
`);
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-[#f2fdfb] to-[#e6f4f1] text-gray-800">
      ...
      <nav className={`${sidebarExpanded ? "w-56" : "w-16"} fixed top-0 left-0 h-full bg-white shadow-lg pt-[70px] transition-all duration-300 overflow-hidden z-40`}>
        <ul className="flex flex-col text-gray-700">
          {[
            ...
            { icon: "fas fa-users", label: "Employees", active: true },
            { icon: "fas fa-bell", label: "Alerts" },
            { icon: "fas fa-user-cog", label: "Settings" }
          ].map(({ icon, label, active }) => (
            <li key={label}>
              <a
                href="#"
                className={`flex items-center gap-3 px-4 py-3 border-l-4 border-transparent hover:bg-teal-100 hover:text-teal-900 transition ${active ? "bg-teal-100 border-teal-700 text-teal-900 font-semibold" : ""}`}
              >
      ...
      {/* MAIN CONTENT */}
      <main className={`${sidebarExpanded ? "ml-56" : "ml-16"} pt-[70px] px-6 md:px-10 pb-8 transition-all duration-300 flex-grow`}>
        ...
        <section className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {employees.map(({ id, name, silo, task, status, remark }) => (
                  <tr key={id} className="even:bg-[#f5fffd] text-center">
                    ...
                    <td className="p-3 border">
                      <button title="Delete Task" className="text-red-600 text-lg hover:text-red-800 transform hover:scale-110 transition">
        ...
        <section className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <form className="space-y-4">
            <div>
              <label className="block font-semibold text-teal-800 mb-1">Employee ID:</label>
              <input type="text" className="w-full border border-gray-300 rounded px-3 py-2" placeholder="Enter Employee ID" />
            </div>
            <div>
              <label className="block font-semibold text-teal-800 mb-1">Task Description:</label>
              <textarea rows="3" className="w-full border border-gray-300 rounded px-3 py-2" placeholder="Describe the task..."></textarea>
            </div>
            <div>
              <label className="block font-semibold text-teal-800 mb-1">Silo Code:</label>
        ...
        <section className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h2 className="text-teal-800 text-xl font-semibold mb-4 flex items-center gap-2">
            <i className="fas fa-bullhorn"></i> Alert & Message System
          </h2>
          <p className="mb-6">Send alerts and messages to employees about critical issues, updates, or tasks.</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Alert Form */}
            <div>
              <h3 className="text-teal-700 font-semibold mb-3">Send New Alert</h3>
              <input type="text" id="alertTitle" placeholder="Enter alert title" className="w-full border border-gray-300 rounded px-3 py-2 mb-4" />
              <textarea id="alertMessage" rows="4" placeholder="Enter your message..." className="w-full border border-gray-300 rounded px-3 py-2 mb-4"></textarea>

              {/* Priority */}
              <label className="block font-semibold text-teal-800 mb-1">Priority:</label>
              ...
              {/* Recipients */}
              <label className="block font-semibold text-teal-800 mb-1">Send To:</label>
        ...
        <section className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-center">
              ...
              <tbody>
                {completedWorks.map(({ id, name, task, silo, date }) => (
